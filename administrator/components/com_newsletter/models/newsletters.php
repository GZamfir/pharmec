...
class NewsletterModelNewsletters extends JModelList
{
    ...
    public function __construct($config = array())
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array('id', 'email', 'published');
        }
    ...
    protected function getListQuery()
    {
        ...
        $query->select('*')
            ->from($db->quoteName('#__newsletter'));

        // Filter: like / search
        $search = $this->getState('filter.search');
        if (!empty($search)) {
            $like = $db->quote('%' . $search . '%');
            $query->where('email LIKE ' . $like.' OR city LIKE'.$like.' OR newsletter_type LIKE'.$like);
        }
        ...
        $orderCol = $this->state->get('list.ordering', 'email');
