...
?>
	...
	var insertColumns = function(editor) {
		layout = getSelectedLayout('layout');
		tag = '<div class="row">';
		jQuery.each(layouts[layout].columns, function(index, value) {
			tag += '<div class="' + value.class + '"><p>Column ' + (index + 1) + '</p></div>';
		});
		tag += '</div>&nbsp;';
	...
	var getSelectedLayout = function(radioName) {
		return jQuery('input[name=' + radioName + ']:checked').val();
	}
<?php
...
<form class="form-horizontal">
	<div class="control-group layouts">
		<?php echo JText::_('Please select a layout'); ?>
		<?php foreach ($layouts as $id => $layout) { ?>
			<div>
				<label for="<?=$id ?>_column"><input type="radio" name="layout" id="<?=$id ?>_column" value="<?=$id ?>" /><?=drawSampleLayout($layout['columns'], 12, 70); ?></label><br/>
			</div>
		<?php
		}
		?>
